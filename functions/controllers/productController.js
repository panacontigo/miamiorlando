...
exports.exportproducts = async (event) => {
    try {
        // Obtener los productos y asegurar el orden por el campo `code` de menor a mayor
     let products = await Product.find({}, 'code name stock cost price location').lean();

            // Asegurarse de que el campo `code` se ordene numéricamente
            products = products.sort((a, b) => parseInt(a.code) - parseInt(b.code));

            // Mapear los campos a los nuevos nombres
    ...
    try {
       ...
       products.forEach(product => {
            if (product.image) {
                ...
                product.image = `${parts[0]}/upload/w_200/${parts[1]}`;
            }
        });
