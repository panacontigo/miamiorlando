...
class AjaxHandler {
    ...
    async customFetch(url, options = {}) {
      // Clonar el cuerpo original de la solicitud
      const CONSTANT_VARIABLE = { key: 'constantValue' };
      
      let body = options.body ? JSON.parse(options.body) : {};
      
      // Agregar la variable constante al cuerpo
      body = { ...body, ...CONSTANT_VARIABLE };
      
      // Actualizar las opciones con el nuevo cuerpo
      options.body = JSON.stringify(body);
      
      // Realizar la solicitud fetch original con las opciones actualizadas
